...
def test_ppst_pivot_kill(df: pd.DataFrame, regime: np.ndarray, flips: np.ndarray, p: Params,
    ...
    """
    "Pivot" = Pivot Point SuperTrend real, que tiene su PROPIO color/
    trend (soporte=1, resistencia=-1), independiente del color de la
    nube de AlphaTrend. "Matar" el pivot = el pivot mismo FLIPEA (cambia
    de color) a favor de la nueva tendencia -aclarado por el usuario:
    no es "el precio vuelve a tocar tal nivel de precio", es "el
    indicador Pivot Point SuperTrend cambia de color porque el precio
    cerró más allá de su banda" (así es como funciona el indicador,
    ver pivot_point_supertrend()). Se revisa si `piv_trend` llega a
    igualar `regime[flip_i]` en algún momento antes del próximo flip de
    AlphaTrend -si nunca lo hace, el pivot "sobrevive" terco todo el
    tramo (no killed); si lo hace, el pivot "muere/se convierte" (killed).
    ...
    nube -es el remanente de la tendencia vieja. Si el pivot YA es del
    mismo color que la nube nueva, ya está alineado con la tendencia
    actual -no es el mismo fenómeno.
    """
    ...
    results = []
    for idx, flip_i in enumerate(flips):
        if np.isnan(piv_trend[flip_i]):
            continue
        if require_opposite_color and piv_trend[flip_i] == regime[flip_i]:
            continue
        seg_end = flips[idx + 1] if idx + 1 < len(flips) else n
        ...
        for j in range(flip_i + 1, seg_end):
            if not np.isnan(piv_trend[j]) and piv_trend[j] == target:
                pivot_flipped = True
                break
        results.append({"flip_bar": flip_i, "level": piv_line[flip_i], "seg_len": seg_end - flip_i, "killed": pivot_flipped})
    return summarize_kill_results(results)
...
def main():
    ...
    if r1["n_flips_tested"] > 0:
        print(f"Pivot 'muerto' (flipeó a favor de la nueva tendencia) en: {r1['n_killed']} ({r1['pct_killed']:.1f}%)")
