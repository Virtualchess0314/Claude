"""
Barrido de parámetros para CRT + FVG, con validación train/test.

Uso:
    python3 optimize.py datos_15m.csv
    python3 optimize.py datos_15m.csv --htf-rule 1h,4h --max-wait-htf-bars 1,2,3
    python3 optimize.py datos_15m.csv --min-gap-atr 0.5,0.75 --min-rr 0.0,1.0,1.5

El CSV pasado es el timeframe MENOR (LTF) — el mayor (HTF) se deriva por
resample adentro del motor (ver data.resample_ohlc), así que `--htf-rule`
tiene que ser múltiplo exacto del timeframe del CSV.

Qué hace:
    1. Carga el CSV (ver data.py).
    2. Parte el historial en train (primer 70%) / test (30% final) en el
       tiempo.
    3. Corre cada combinación de parámetros en train y en test.
    4. Descarta combinaciones con menos de --min-trades operaciones en test.
    5. Ordena por la métrica pedida en TEST.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("csv_path", help="CSV de velas del timeframe MENOR (ver data.py)")
    ...
    ap.add_argument("--htf-rule", type=parse_str_list, default=["1h"])
    ap.add_argument("--max-wait-htf-bars", type=parse_int_list, default=[1, 2, 3])
    ap.add_argument("--min-gap-atr", type=parse_float_list, default=[0.5, 0.75])
    ...
    ap.add_argument("--min-rr", type=parse_float_list, default=[0.0, 1.0, 1.5])
...
def run_grid(df_train: pd.DataFrame, df_test: pd.DataFrame, args) -> pd.DataFrame:
    ...
    combos = list(
        itertools.product(
            args.htf_rule, args.max_wait_htf_bars, args.min_gap_atr, args.min_body_ratio,
            args.min_range_atr, args.touch_tol_atr, args.sl_buffer_atr, args.min_rr,
        )
    )
    print(f"Probando {len(combos)} combinaciones...", file=sys.stderr)

    for (htf_rule, max_wait_htf_bars, min_gap_atr, min_body_ratio,
         min_range_atr, touch_tol_atr, sl_buffer_atr, min_rr) in combos:
        p = Params(
            htf_rule=htf_rule, max_wait_htf_bars=max_wait_htf_bars,
            min_gap_atr=min_gap_atr, min_body_ratio=min_body_ratio, min_range_atr=min_range_atr,
            touch_tol_atr=touch_tol_atr, sl_buffer_atr=sl_buffer_atr, min_rr=min_rr,
...
def main():
    ...
    if results.empty:
        print(
            ...
            "historial, o subí --max-wait-htf-bars.",
            file=sys.stderr,
        )
        return
